	...
	function window:open()
		...
		for i = 1, #descendants do
			...
			if descendant.ClassName == "Frame" then
				...
				util:tween(descendant, twinfo(0.24, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = (descendant.BackgroundColor3 == Color3.fromRGB(1,1,1) and 0.5 or 0)})
...
function window:new_tab(text)
    ...
    local on_accent_change = util:create_connection(lib.on_accent_change, function(color)
        if self.active_tab == text then
            ...
            UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, color), ColorSequenceKeypoint.new(0.10, lib.accent_color), ColorSequenceKeypoint.new(0.20, Color3.fromRGB(32, 32, 32)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(32, 32, 32))};
		end
    end)
...
function tab:set_active()
	...
	fix2.Visible = true
	frame.Visible = true
    gradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, lib.accent_color), ColorSequenceKeypoint.new(0.10, lib.accent_color), ColorSequenceKeypoint.new(0.20, Color3.fromRGB(32, 32, 32)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(32, 32, 32))};
...
function tab:new_subtab(text)
	...
	local on_hover = util:create_connection(SubtabButton.MouseEnter, function(input)
		if self.active_subtab ~= text then
			ButtonLabel.TextColor3 = Color3.fromRGB(126,126,126)
		end
	end)
...
lib.new = LPH_JIT(function()
    ...
    local on_accent_change = util:create_connection(lib.on_accent_change, function(color)
        ...
        new_window:set_build("debug")
        new_window:set_title("ratio.lol")
    end)
...
do
	...
	aimbot.get_closest = LPH_JIT(function()
		...
		local folder_name = lplr.Character.Parent.Name == "Zombies" and "Survivors" or "Zombies"
		...
		for name, player in pairs(all_players) do
			...
			if hrp and hum then
				if player.health > 0 and player.alive and (hrp.Position-lplr_pos).magnitude < max_distance then
					if hrp.Parent then
						local parent_parent = hrp.Parent.Parent
						if parent_parent then
							if parent_parent.Name ~= folder_name then
								continue
							end
						end
					end
					...
					if distance < closest_distance then
						...
						if visible_check then
							if not world.is_visible(hrp.Position, lplr_hrp.Position, {hrp, hrp.Parent, lplr.Character, workspace:FindFirstChild("NoRay"), workspace:FindFirstChild("Map") and workspace.Map:FindFirstChild("NoRay") or nil}) then
								continue
							end
						end
						closest_distance = distance
	...
	aimbot.get_aimbot_location = LPH_JIT(function(character)
		local part = flags["part"]["selected"][1]
		part = (part == "Closest" and aimbot.get_closest_part(character)) or (part == "Root" and character:FindFirstChild("HumanoidRootPart")) or (part == "Head" and character:FindFirstChild("Head"))
...
do
	...
	local on_character_added = LPH_JIT(function(character)
		...
		util:create_connection(humanoid.HealthChanged, LPH_NO_VIRTUALIZE(function(health)
			...
			character_table.Health = old_health
...
do
	...
	function client.on_character_added(character)
		...
		if flags["forcefield_body"]["toggle"] then
			...
			for i = 1, #parts do
				local part = parts[i]
				if part.Name:find("Armor") then return end
		...
		task.spawn(LPH_NO_VIRTUALIZE(function()
			...
			for _, func in pairs(getgc()) do
				if typeof(func) == "function" and getinfo(func, "n").name == "removestamina" then
					old = nil; old = hookfunction(func, function(...)
						if flags["infinite_stamina"]["toggle"] then return end
						return old(...)
					end)
					return
				end
...
local new_watermark = watermark.new(lower(lplr.Name), "ready 2 die")
...
local window = lib.new(); window:set_accent_color(Color3.fromRGB(189, 172, 255))
...
local combat = window:new_tab("Combat")
    local combat_aimbot = combat:new_subtab("Aimbot")
		local aimbot_general = combat_aimbot:new_section({name = "General", side = "left", size = 360})
			local aimbot_enabled = aimbot_general:new_element({name = "Silent aim", flag = "aimbot", types = {toggle = {}, keybind = {method = "always"}}}) keybind.new("Silent aim", aimbot_enabled, "aimbot")
			local part = aimbot_general:new_element({name = "Part", flag = "part", types = {dropdown = {no_none = true, default = {"Closest"}, options = {"Head", "Torso", "Closest"}}}})
			local ignore_if = aimbot_general:new_element({name = "Ignore if", flag = "ignore_if", types = {dropdown = {options = {"Not visible", "Forcefield"}, multi = true}}})
			local max_distance = aimbot_general:new_element({name = "Max distance", flag = "max_distance", types = {slider = {min = 75, max = 25000, suffix = "m", default = 10000}}})
...
local player_ = window:new_tab("Player")
	local player_general = player_:new_subtab("General")
        local general_modifiers = player_general:new_section({name = "Modifiers", side = "left", size = 360})
			local infinite_stamina = general_modifiers:new_element({name = "Infinite stamina", flag = "infinite_stamina", types = {toggle = {}}})
            local no_fall_damage = general_modifiers:new_element({name = "No fall damage", flag = "no_fall_damage", types = {toggle = {}}})
			local no_trip = general_modifiers:new_element({name = "No trip", flag = "no_trip", types = {toggle = {}}})
		local movement_ = player_general:new_section({name = "Movement", side = "right", size = 360})
...
local visuals = window:new_tab("Visuals")
	local visuals_players = visuals:new_subtab("Players")
		local players_target = visuals_players:new_section({name = "Target esp", side = "left", size = 200})
	...
	local visuals_other = visuals:new_subtab("Other")
		local other_game = visuals_other:new_section({name = "Game", side = "left", size = 200})
		do
...
do
	local old_namecall = nil; old_namecall = hookmetamethod(game, "__index", LPH_NO_VIRTUALIZE(function(self, ...)
		local args = {...}
        if not checkcaller() then
			local method = getnamecallmethod()
			if method == "FireServer" and typeof(self) == "Instance" and self:IsA("RemoteEvent") then
				local remote_name = tostring(self)
				if remote_name == "SelfDamage" then
					if flags["no_fall_damage"]["toggle"] then
						if typeof(args[1]) == "number" then
							return
						end
					end
				elseif remote_name == "Touched" then
					if flags["no_trip"]["toggle"] then
						if tostring(args[1]) == "Trip" then
							return
						end
					end
				end
    ...
    local old_index = nil; old_index = hookmetamethod(game, "__index", LPH_NO_VIRTUALIZE(function(self, index)
		...
		elseif not checkcaller() then
			if lower(index) == "unitray" then
				if aimbot.aimbot_location then
					return ray_new(
						self.Origin.Position,
						(aimbot.aimbot_location - self.Origin.Position).Unit
					)
				end
			end
        end
	...
	local new_index = nil; new_index = hookmetamethod(game, "__newindex", LPH_NO_VIRTUALIZE(function(self, index, value)
		if not checkcaller() then
			...
			elseif typeof(self) == "Instance" then
				if self == lplr and index == "CameraMaxZoomDistance" then
...
do -- chat did he eat
	...
	local fov_circle, fov_outline, tracer_line, bounding_box, tracer_circle = drawing_cache.fov, drawing_cache.fov_outline, drawing_cache.target_tracer, drawing_cache.bounding_box, drawing_cache.tracer_circle
	...
	local on_heartbeat = util:create_connection(rs.Heartbeat, LPH_NO_VIRTUALIZE(function(dt)
		...
		if hrp and hum then
			...
			if flags["aimbot"]["toggle"] and aimbot_enabled:is_active() then
				...
				if closest and closest:FindFirstChild("Humanoid") then
					...
					if flags["target_tracer"]["toggle"] then
						...
						if on_screen then
							local pos2, on_screen2 = camera:WorldToViewportPoint(closest.HumanoidRootPart.Position)
							if on_screen2 then
			...
			after_render:Fire()
		end; tracer_line.Visible = tracer_line_visible; aimbot.aimbot_location = aimbot_location; aimbot.active_target = closest; bounding_box.Visible = bounding_box_visible; tracer_circle.Visible = tracer_circle_visible
	end))
end
