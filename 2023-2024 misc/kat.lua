...
function section:new_element(info)
	...
	for element, info in pairs(types) do
		if element == "toggle" then 
			...
			function new_element:set_toggle(toggle)
			...
			if default then new_element:set_toggle(default) end
		...
		elseif element == "slider" then
			...
			local on_input_began = util:create_connection(SliderBackground.InputBegan, function(input)
				if input.UserInputType == Enum.UserInputType.MouseButton1 and not lib.busy then
					lib.busy = true
					local distance = math.clamp((mouse.X - SliderBackground.AbsolutePosition.X)/SliderBackground.AbsoluteSize.X, 0, 1)
		...
		elseif element:find("colorpicker") then
            ...
            local function update_sv(val, sat, nocallback)
                ...
                SaturationMover.Position = UDim2.new(math.clamp(sat/255, 0, 0.98), 0, 1 - math.clamp(val/255, 0.02, 1), 0)
            ...
            local function update_hue(hue2)
                ...
                HueMover.Position = UDim2.new(0, -1, math.clamp(hue2/360, 0, 0.99), -1)
            ...
            local function update_transparency(o, nocallback)
                TransparencyMover.Position = UDim2.new(math.clamp(1 - o, 0, 0.98), 0, 0, -1)
            ...
            util:create_connection(SaturationImage.InputBegan, function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
                    local xdistance = math.clamp((mouse.X - SaturationImage.AbsolutePosition.X)/SaturationImage.AbsoluteSize.X, 0, 1)
                	local ydistance = 1 - math.clamp((mouse.Y - SaturationImage.AbsolutePosition.Y)/SaturationImage.AbsoluteSize.Y, 0, 1)
            ...
            util:create_connection(mouse.Move, function()
                ...
                elseif dragging_hue then
                    local xdistance = math.clamp((mouse.Y - HueFrame.AbsolutePosition.Y)/HueFrame.AbsoluteSize.Y, 0, 1)
                    local hue = 360 * xdistance
                    update_hue(hue)
                elseif dragging_trans then
                    local xdistance = math.clamp((mouse.X - TransparencyFrame.AbsolutePosition.X)/TransparencyFrame.AbsoluteSize.X, 0, 1)
...
local window = lib.new(""); window:set_user(game.Players.LocalPlayer.Name)
	local combat = window:new_tab("Combat")
		local general = combat:new_subtab("General")
			local aimbot_section = general:new_section({name = "Aimbot", side = "left", size = 360})
				...
				local randomization = aimbot_section:new_element({name = "Randomization", flag = "randomization", tip = "Adds a random offset to the silent aim position, higher values equals more misses", types = {slider = {min = 1, max = 100, default = 1, suffix = "%"}}}); set_dependent(randomization, aimbot_enabled)
			...
			local triggerbot_section = general:new_section({name = "Triggerbot", side = "left", size = 360})
				local triggerbot = triggerbot_section:new_element({name = "Enabled", flag = "triggerbot", types = {toggle = {}}})
				local triggerbot_delay = triggerbot_section:new_element({name = "Delay", flag = "triggerbot_delay", types = {slider = {min = 1, max = 300, suffix = "ms"}}}); set_dependent(triggerbot_delay, triggerbot)
	...
	local visuals = window:new_tab("Visuals")
		...
		local other_section = visuals:new_subtab("Other")
			local other_hud = other_section:new_section({name = "Hud", side = "left", size = 360})
...
local function is_visible(start, result, part)
    return #camera:GetPartsObscuringTarget({start, result}, {lplr.Character, part, part.Parent, workspace.WorldIgnore}) == 0
end

-- ? Hack Functions

getgenv().mouse1click = function() 
	for i,v in pairs(getconnections(uis.InputBegan)) do
		v.Function(false, Enum.UserInputType.MouseButton1)
	end
end

local function get_closest_part(character) 
    local parts_to_check = {"Head", "HumanoidRootPart", "Left Leg", "Right Leg", "Left Arm", "Right Arm"}
				...
				if hrp and hum then
					...
					if visible then
						...
						if distance <= threshold then
                            if not ignore and forced_target == plr then return character end
                            if distance < closest then
                                target = character
                                closest = distance
                            end
...
local function do_triggerbot(time)
	aimbot.triggerbot_delay = true
	task.wait(time/1000)
	for i,v in pairs(getconnections(mouse.Button1Down)) do
		v:Fire({})
	end
	task.wait()
	for i,v in pairs(getconnections(mouse.Button1Up)) do
		v:Fire({})
	end
	aimbot.triggerbot_delay = false
end
...
local on_heartbeat = util:create_connection(rs.Heartbeat, function(dt)
	...
	if character then
		...
		if hrp then
			aimbot.active_target = get_closest() or nil
			...
			if flags["speed"]["toggle"] and cframe_speed:is_active() then
				hrp.CFrame = hrp.CFrame + character.Humanoid.MoveDirection * (lib.flags["speed"]["value"]/200)
			end
			if is_spinbot then
				hrp.CFrame = hrp.CFrame * CFrame.Angles(0,math.rad(1+flags["spinbot"]["value"]),0)
			end
		end
		if flags["triggerbot"]["toggle"] and not aimbot.triggerbot_delay then
			local target = mouse.Target
			local revolver = character:FindFirstChild("Revolver")
			if revolver and target and not target:IsDescendantOf(character) then
				local parent = target.Parent
				local next_parent = parent.Parent
				local trigger_delay = flags["triggerbot_delay"]["value"]
				if next_parent and next_parent:FindFirstChildOfClass("Humanoid") and next_parent.Parent ~= workspace.Ragdolls then
					if not next_parent:FindFirstChildOfClass("ForceField") then
						task.spawn(do_triggerbot, math.random(math.clamp(trigger_delay*.8, 1, 999), trigger_delay))
					end
				elseif parent and parent:FindFirstChildOfClass("Humanoid") and parent.Parent ~= workspace.Ragdolls then
					if not parent:FindFirstChildOfClass("ForceField") then
						task.spawn(do_triggerbot, math.random(math.clamp(trigger_delay*.8, 1, 999), trigger_delay))
					end
				end
			end
	...
	for _, player in pairs(all_players) do
		local hrp = player:is_character_loaded()
		player.is_loaded_cache = hrp and hrp or false
		if hrp then
			if not player.last_position then player.last_position = hrp.Position; continue end
			player.Velocity = (hrp.Position - player.last_position) / dt
			player.last_position = hrp.Position
		end
	end
end)
		...
		if hrp then
			...
			if visible then		
				...
				if hflag["toggle"] then
					...
					highlight.Adornee = instance.Character
				...
				if offscreen["toggle"] then
					...
					local big, small = math.max(screen_size.x, screen_size.y), math.min(screen_size.x, screen_size.y)
			...
			local last_position = player.last_position
			if not last_position then continue end
			local pos, visible = wtvp(last_position)

			if visible and drawings.name.Transparency ~= 0 then		
				local size = (wtvp(last_position - Vector3.new(0, 3.3, 0)).Y - wtvp(last_position + Vector3.new(0, 2.9, 0)).Y) / 2
				local box_size = Vector2.new(math.floor(size * 1.4), math.floor(size * 1.9))
				local box_pos = Vector2.new(math.floor(pos.X - size * 1.4 / 2), math.floor(pos.Y - size * 1.6 / 2))
...
local function get_aimbot_location(character)
	local flags = lib.flags
	local part = flags["aimbot_part"]["selected"][1]
	if part == "Random" then
	...
	local random = flags["randomization"]["value"]
	local position = part.Position + Vector3.new((math.random(0, random)/50) * (math.random(1,2) == 1 and -1 or 1), (math.random(0, random)/50) * (math.random(1,2) == 1 and -1 or 1), (math.random(0, random)/50) * (math.random(1,2) == 1 and -1 or 1))
	local is_vis = is_visible(position, lplr.Character.Head.Position, part)
	return (is_vis and position or false)
end
...
local old_namecall = nil; old_namecall = hookmetamethod(game, "__namecall", function(self, ...)
	...
	if not checkcaller() and method == "FindPartOnRayWithIgnoreList" and (tostring(script) == "RevolverClient" or tostring(script) == "KnifeClient") then
		if aimbot_enabled:is_active() and aimbot.active_target then
			local aimbot_location = nil
			task.spawn(function()
				aimbot_location = get_aimbot_location(aimbot.active_target)
			end)
			if aimbot_location then
				args[1] = Ray.new(camera.CFrame.p, (((aimbot_location - camera.CFrame.p).unit) * 999))
