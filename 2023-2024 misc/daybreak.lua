...
local cache = {
	...
	map = workspace.Map_Holder:FindFirstChild("Game_Map") or nil 
}
-- ? UI Creation
...
local window = lib.new(""); window:set_user(game.Players.LocalPlayer.Name)
	...
	local visuals = window:new_tab("Visuals")
		local players_section = visuals:new_subtab("Players")
			local enemy_esp = players_section:new_section({name = "Survivor esp", side = "left", size = 360})
			do
				local esp_enabled = enemy_esp:new_element({name = "Enabled", flag = "esp", types = {toggle = {}}})
				local esp_offscreen = enemy_esp:new_element({name = "Offscreen", flag = "offscreen", types = {toggle = {}, colorpicker = {color = Color3.fromRGB(0,255,0), transparency = 0.8}}})
				...
				local esp_highlight = enemy_esp:new_element({name = "Highlight", flag = "highlight", types = {toggle = {}, colorpicker = {color = Color3.fromRGB(0,255,0), transparency = 0.5}}})
			...
			local killer_esp = players_section:new_section({name = "Killer esp", side = "right", size = 360})
			do
				local esp_enabled = killer_esp:new_element({name = "Enabled", flag = "killer_esp", types = {toggle = {}}})
				local esp_offscreen = killer_esp:new_element({name = "Offscreen", flag = "killer_offscreen", types = {toggle = {}, colorpicker = {color = Color3.fromRGB(255,0,0), transparency = 0.8}}})
				local offscreen_distance = killer_esp:new_element({name = "Distance", flag = "killer_offscreen_distance", types = {slider = {min = 1, max = 1000}}}); set_dependent(offscreen_distance, esp_offscreen)
				local offscreen_size = killer_esp:new_element({name = "Size", flag = "killer_offscreen_size", types = {slider = {min = 1, max = 100}}}); set_dependent(offscreen_size, esp_offscreen)
				local esp_highlight = killer_esp:new_element({name = "Highlight", flag = "killer_highlight", types = {toggle = {}, colorpicker = {color = Color3.fromRGB(255,0,0), transparency = 0.5}}})
				local esp_outline = killer_esp:new_element({name = "Outline", flag = "killer_outline", types = {colorpicker = {color = Color3.fromRGB(0,0,0)}}}); set_dependent(esp_outline, esp_highlight)
				local esp_name = killer_esp:new_element({name = "Name", flag = "killer_name", types = {toggle = {}, colorpicker = {color = Color3.fromRGB(255,0,0)}}})
				local esp_box = killer_esp:new_element({name = "Box", flag = "killer_box", types = {toggle = {}, colorpicker = {color = Color3.fromRGB(255,0,0)}}})
			end
		local other_section = visuals:new_subtab("Other")
			local other_hud = other_section:new_section({name = "Hud", side = "Right", size = 360})
				local killer_warning = other_hud:new_element({name = "Killer warning", flag = "killer_warning", types = {toggle = {}}})
			...
			local generator_esp = other_section:new_section({name = "Generator esp", side = "right", size = 360})
				local generator_highlight = generator_esp:new_element({name = "Highlight", flag = "generator_highlight", types = {toggle = {}, colorpicker = {color = Color3.fromRGB(255,0,0), transparency = 0.5}}})
				local generator_outline = generator_esp:new_element({name = "Outline", flag = "generator_outline", types = {colorpicker = {color = Color3.fromRGB(255,0,0), transparency = 0.5}}}); set_dependent(generator_outline, generator_highlight)
				local generator_text = generator_esp:new_element({name = "Text", flag = "generator_text", types = {toggle = {}, colorpicker = {color = Color3.fromRGB(255,255,255)}}})
				local generator_progress = generator_esp:new_element({name = "Show progress", flag = "generator_progress", types = {toggle = {}}}); set_dependent(generator_progress, generator_text)
...
cache.killer_warning = draw:new("Text", {
	Center = true,
	Outline = true,
	Size = 18,
	ZIndex = 2,
	Text = "THE KILLER CAN SEE YOU!",
...
local function is_visible(start, result, part, other)
    return #camera:GetPartsObscuringTarget({start, result}, {lplr.Character, part, part.Parent, cache.ignored, other}) == 0
end

local gen_cache = {}

local function do_gen_esp(gen)
	local 
end

local function do_map_setup(map)
	cache.ignored = map:WaitForChild("barriers")

	local gens = map:WaitForChild("Gens")
...
do
	local killer_warning = cache.killer_warning
		...
		if character then
			...
			if hrp and humanoid then
				...
				if flags["speed"]["toggle"] and cframe_speed:is_active() then
					hrp.CFrame = hrp.CFrame + character.Humanoid.MoveDirection * (flags["speed"]["value"]/100)
				end
				if flags["fly"]["toggle"] and cframe_fly:is_active() then
					local vel = hrp.Velocity
					local add = character.Humanoid.MoveDirection * (flags["fly"]["value"]/100)
	...
	local on_render = util:create_connection(rs.RenderStepped, function()
		...
		for i = 1, #cache2 do
			...
			if hrp then
				local prefix = instance.Character:FindFirstChild("Weapon") and "killer_" or ""

				local pos, visible = wtvp(hrp.Position)

				local can_killer_see = nil

				if prefix == "killer_" then
					can_killer_see = is_visible(hrp_pos, lplr.Character.HumanoidRootPart.Position, hrp)
					if flags["killer_warning"]["toggle"] then
						if can_killer_see then
							local viewport_size = camera.ViewportSize
							killer_warning.Visible = true
							killer_warning.Position = Vector2.new((viewport_size.X / 2) - (killer_warning.TextBounds.X / 2), viewport_size.Y / 2 - 9)
						end
					end
				end

				if visible then		
					local size = (wtvp(hrp.Position - vect3(0, 3.4, 0)).Y - wtvp(hrp.Position + vect3(0, 2.9, 0)).Y) / 2
					local box_size = vect2(floor(size * 1.5), floor(size * 1.9))
					local box_pos = vect2(floor(pos.X - size * 1.5 / 2), floor(pos.Y - size * 1.6 / 2))

					local hflag = flags[prefix.."highlight"]
					local character = instance.Character

					if hflag["toggle"] then
						local oflag = flags[prefix.."outline"]
					...
					local offscreen = flags[prefix.."offscreen"]
					if offscreen["toggle"] then
						...
						local arrow_origin = screen_size/2 + vect2(cx * big * 75/200, sy * small * 75/200) * flags[prefix.."offscreen_distance"]["value"]/1000

						local arrow_size = flags[prefix.."offscreen_size"]["value"]/100
